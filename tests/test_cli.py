...
TEST_NOTEBOOKS = {
    ...
    'j1-nbi':
        join(TEST_NOTEBOOKS_FOLDER, 'basic_j1-nbi.ipynb'),
    'cleared_interact':
        join(TEST_NOTEBOOKS_FOLDER, 'cleared_interact.ipynb'),
    'cleared_j1-nbi':
        join(TEST_NOTEBOOKS_FOLDER, 'cleared_j1-nbi.ipynb'),
    'images':
...
WIDGET_BUTTON_RE = re.compile(r'<button class="js-j1-nbi-widget">')
...
class TestCli(object):
    ...
    def test_spec(self):
        """
        Tests that spec is set correctly in j1-nbi initialization
        """
    ...
    def test_plain_template(self):
        """
        Tests that j1-nbi is not loaded in the plain template but that
        ...
        with convert_one(
            ...
            assert j1-nbi_SCRIPT not in html
            assert WIDGET_BUTTON_RE.search(html)

    def test_partial_template(self):
        """
        Tests that j1-nbi is loaded in the partial template and that the
        ...
        with convert_one(
            ...
            html = ''.join(f.readlines())
            assert j1-nbi_SCRIPT in html
            assert '<html>' not in html
            assert WIDGET_BUTTON_RE.search(html)

    def test_full_template(self):
        """
        Tests that j1-nbi is loaded in the full template and that the
    ...
    def test_output(self, tmpdir):
        ...
        """
        with convert_many(
            [TEST_NOTEBOOKS['j1-nbi'], TEST_NOTEBOOKS['interact']], {
                '--output': str(tmpdir)
            }
        ) as html_files:
            assert all(
    ...
    def test_folder(self):
        ...
        """
        expected_files = {
            'empty.html', 'basic_interact.html', 'basic_j1-nbi.html',
            'cleared_interact.html', 'cleared_j1-nbi.html',
