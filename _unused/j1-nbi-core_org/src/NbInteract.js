...
const DEFAULT_SPEC = 'SamLau95/j1-nbi-image/master'

/**
 * Main entry point for j1-nbi.
 *
 * Class that runs notebook code and creates widgets.
 */
export default class j1-nbi {
  /**
   * Initialize j1-nbi. Does not start kernel until run() is called.
   *
   * @param {Object} [config] - Configuration for j1-nbi
   *
   ...
   *     If set, j1-nbi ignores all Binder config and will directly request
  ...
  async prepare() {
    ...
    // point, j1-nbi is ready to run so we change the button text to match.
