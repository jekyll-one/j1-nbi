...
This file defines the Exporter for the j1-nbi conversion utility. After
installing this package, the user can run

    j1-nbi some_notebook.ipynb
...
<a href="https://www.j1-nbi.com/tutorial/tutorial_publishing.html"
        target="_blank">
    https://www.j1-nbi.com/tutorial/tutorial_publishing.html
</a>
'''
...
https://github.com/SamLau95/j1-nbi/issues/new and include the error output
below:
    ...
    def __init__(self, config=None, **kw):
        ...
        Kwargs:
            ...
            spec (str): BinderHub spec for Jupyter image. Must be in the
                ...
                `main`. Defaults to 'SamLau95/j1-nbi-image/master'.
            ...
            template_file (str):
                ...
                styling. Automatically loads the j1-nbi JS library.
                ...
                another page. Automatically loads the j1-nbi JS library.

                'plain': Outputs an HTML partial to embed in another page. The
                partial does not include the j1-nbi JS library; the
                embedding page must load and initialize the j1-nbi JS
                library itself.
...
def publish(spec, nb_name, template='full', save_first=True):
    ...
    Equivalent to running `j1-nbi ${spec} ${nb_name}` on the command line.
    ...
    Kwargs:
        template (str): Template to use for conversion. Valid templates:
            ...
            - 'full': Outputs a complete standalone HTML page with default
              styling. Automatically loads the j1-nbi JS library.
            - 'partial': Outputs an HTML partial that can be embedded in
              another page. Automatically loads the j1-nbi JS library but
              has no styling.
            - 'plain': Outputs an HTML partial used to embed in an HTML page
              where the j1-nbi JS library is already loaded. Does not load
    ...
    try:
        check_output(
            ['j1-nbi', '--template', template, '--spec', spec, nb_name],
            stderr=STDOUT
        )
