...
Usage:
  j1-nbi init
  j1-nbi NOTEBOOKS ...
  j1-nbi [options] NOTEBOOKS ...
  j1-nbi (-h | --help)

`j1-nbi init` initializes a GitHub project for j1-nbi. It
provides guided help to set up a requirements.txt file (if needed) and a Binder
image for the project.

`j1-nbi NOTEBOOKS ...` converts notebooks into HTML pages. Note that
running this command outside a GitHub project initialized with `j1-nbi
...
Options:
  ...
  -s SPEC --spec SPEC        BinderHub spec for Jupyter image. Must be in the
                             ...
                             'SamLau95/j1-nbi-image/master'. This flag is
                             **required** unless a .j1-nbi.json file exists
...
def binder_spec_from_github_url(github_url):
    ...
    For example:
    git@github.com:SamLau95/j1-nbi.git -> SamLau95/j1-nbi/master
...
def yes_or_no(question, default="yes"):
    ...
    while True:
        sys.stdout.write(
            '{}[j1-nbi]{} {}{}'.format(BLUE, NOCOLOR, question, prompt)
        )
        choice = input().lower()
...
def main():
    """
    Parses command line options and runs j1-nbi.
    """
...
def init():
    ...
    if os.path.isdir('.git'):
        ...
        error(
            ...
            "Please rerun j1-nbi init in the top-level folder of a "
            "git project."
        )
        return ERROR
    log()
    ...
    if os.path.isfile('Dockerfile'):
        ...
        if yes:
            ...
            log(
                ...
                'rest of your dependencies, then rerun j1-nbi init.'
            )
            return SUCCESS
        else:
            log(
    ...
    log('Generating .j1-nbi.json file...')
    if os.path.isfile(CONFIG_FILE):
        log(
            ".j1-nbi.json already exists, skipping generation. If you'd "
            "like to regenerate the file, remove .j1-nbi.json and rerun "
            "this command."
        )
        log()
    ...
    if 'github' not in github_origin:
        error(
            ...
            "your .j1-nbi.json file after this script finishes. "
    ...
    log('Created .j1-nbi.json file successfully')
    log()
...
def check_arguments(arguments):
    if not arguments['--spec']:
        error(
            '--spec flag not set and no .j1-nbi.json file found. Rerun '
            'this command with the --spec flag or run `j1-nbi init` to '
            'resolve this issue.'
        )
        raise DocoptExit()
...
def init_exporter(extract_images, execute, **exporter_config):
    ...
    if execute:
        ...
        # for interact() calls.
        preprocessors.append('j1-nbi.preprocessors.NbiExecutePreprocessor')
