{#
This template doesn't load the j1-nbi JS since it assumes that the
embedding page loads and sets up j1-nbi.

This page produces basically the same output as

    jupyter nbconvert --to html --template basic

But replaces widgets with buttons to start j1-nbi.
#}
...
{% set j1-nbi_class = 'js-j1-nbi-widget' %}

{% set j1-nbi_button_text = 'Loading widgets...' %}

{#
Keep classes in sync with j1-nbi_css.tpl.

The HIDDEN marker is for backwards-compatibility; nbi:hide_in is preferred now
#}
{% set j1-nbi_markers = [
  {'marker': 'nbi:left', 'cls': 'j1-nbi-left'},
  {'marker': 'nbi:right', 'cls': 'j1-nbi-right'},
  {'marker': 'nbi:hide_in', 'cls': 'j1-nbi-hide_in'},
  {'marker': 'nbi:hide_out', 'cls': 'j1-nbi-hide_out'},
  {'marker': 'HIDDEN',  'cls':'j1-nbi-hide_in'},
] %}
{% set j1-nbi_default_cell_cls = 'j1-nbi-row' %}
...
{% block codecell %}
  ...
  {% set cell_classes = j1-nbi_markers | selectattr("marker", "in", cell.source) | join(' ', attribute='cls') %}

  <div class="{{ cell_classes | default(j1-nbi_default_cell_cls) }}
